...
class CNNEmulator(eqx.Module):
    ...
    def __init__(self, key: PRNGKeyArray):
        layers = []
        layers.append(eqx.nn.Conv2d(2, 4, 3, padding=[1,1], key = key))
        layers.append(eqx.nn.Lambda(jnp.tanh))
        for i in range(3):
            key, subkey = jax.random.split(key)
            # Take two frames then predict the next one
            layers.append(eqx.nn.Conv2d(4, 4, 3, padding=[1,1], key = subkey))

            layers.append(eqx.nn.Lambda(jnp.tanh))
        layers.append(eqx.nn.Conv2d(4, 1, 3, padding=[1,1], key = key))
        self.layers = layers

    def __call__(self, x: Float[Array, " n_res n_res"]) -> Float[Array, " n_res n_res"]:
